...
describe "MockableHttpServer", () ->
  ...
  describe "dispatch", () ->
    ...
    it "proxies for forward route", () ->
      request.url = "/forward"
      forward = forwardEntry.forward
      target = "http://#{forward.host}:#{forward.port}"
      tested.dispatch(request, response, proxy)
      expect(mockProxyServer.web).toHaveBeenCalledWith(request, response,
        {target: target})
    ...
    it "decrements times", () ->
      ...
      expect(response.write).toHaveBeenCalledWith(
        responseTimesEntry.response.body)
      expect(response.end).toHaveBeenCalledWith
  ...
  describe "logged requests", () ->
    ...
    it "not found immediatelly", () ->
      ...
      callback = setIntervalCallback.calls.first().args[0]
      callback()
      expect(reject).toHaveBeenCalledWith "Did not get logs of anything " + \
        "after timeout of 0"
